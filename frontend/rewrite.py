...
# Find the exact insertion point using regex
match = re.search(r'\s+if\s*\(loading\)\s*return', content)
if not match:
    print("Could not find 'if (loading) return', aborting.")
    exit(1)

split_idx = match.start()
top_half = content[:split_idx]
...
# Check if connectionType is already in top_half
if 'const [connectionType' not in top_half:
    state_vars += "\n    const [connectionType, setConnectionType] = useState<'cloud' | 'server'>('cloud');\n"

bottom_content = r'''    if (loading) return <div className="animate-pulse flex space-x-4">{tc.loading}</div>;
                ...
                <div className="px-6 py-4 border-b border-border bg-emerald-500/5 flex items-center justify-between">
                    <div>
                        <h2 className="text-lg font-semibold flex items-center gap-2 text-emerald-600">
                            <Globe size={18} />
                            Global Jira Credentials
                            {globalStatus?.type === 'success' && <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] uppercase font-bold bg-green-500/10 text-green-500 border border-green-500/20"><CheckCircle2 size={12}/> Connected</span>}
                            {globalStatus?.type === 'error' && <span className="ml-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] uppercase font-bold bg-red-500/10 text-red-500 border border-red-500/20"><AlertCircle size={12}/> Not Found</span>}
                        </h2>
                                            ...
                                            <div className="space-y-4">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                                    {(['BALANCED', 'SPEED', 'QUALITY', 'GROWTH', 'PERSONALIZED'] as const).map((id) => {
                                                        const strategy = tc.strategies[id];
                                                        return (
                                                            <label
                                                                key={id}
                                                            ...
                                                            >
                                                                <input type="radio" name="active_strategy" value={id} className="sr-only" checked={formData.active_strategy === id} onChange={handleChange} />
                                                                <span className="flex flex-col flex-1">
                                                                    <span className={`block text-xs font-bold ${formData.active_strategy === id ? 'text-primary' : 'text-foreground'}`}>{strategy.label}</span>
                                                    ...
                                                    <div className="mt-6 space-y-4 animate-in slide-in-from-top-1 fade-in duration-200">
                                                        ...
                                                        <div className="flex items-center justify-between pt-2">
                                                            ...
                                                            <button type="button" onClick={async () => {
                                                                ...
                                                                try {
                                                                    await api.post('http://localhost:8000/api/settings/advanced', advSettings);
                                                                ...
                                                                {advSaving ? tc.advSaving : tc.save}
                                                            </button>
                                                        </div>
                                                    </div>
                                                )}
...
final_content = top_half + state_vars + '\n' + bottom_content
